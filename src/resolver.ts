import {
  ...
  insertBook,
  removeBook,
} from "./business-logic/books-logic";
import {
  getAllUsers,
  getSingleUser,
  insertUser,
  logUserIn,
} from "./business-logic/users-logic";
...
const resolvers = {
  ...
  Mutation: {
    async createBook(
      root,
      { title, author, pageCount, genre, publisher, description },
      context
    ) {
      if (!context.user) {
        return null;
    ...
    async createUser(root, { email, name, password }) {
      const newUser: any = await insertUser({ email, name, password });
      return newUser;
    },
